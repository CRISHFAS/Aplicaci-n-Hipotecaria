...
def create_amortization_table(amount, rate, term):
    ...
    # Calculate Amount using the formula for an annuity
    ...
    for payment_number in range(1, total_payments + 1):
        ...
        amortization_table.append([payment_number, monthly_payment, principal_payment, interest_payment, remaining_balance])

    # Create DataFrame for better visualization
    amortization_df = pd.DataFrame(amortization_table, columns=['Payment', 'Amount', 'Principal Payment', 'Interest Payment', 'Remaining Balance'])
    
    return amortization_df

def create_payment_schedule_to_edit(amortization_table):
    payment_schedule = amortization_table[['Payment', 'Amount']].round(2)
